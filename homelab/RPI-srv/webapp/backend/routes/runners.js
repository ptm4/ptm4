...
// Renamed from agents.js 2026-07-25: these are "runners" (the 4 scheduled collectors that
// produce reports — homelab-doctor, hardware, software, network) surfaced on the Reports
// tab. "Agent" now means a per-host installed service (see routes/architecture.js's
// /ingest — the first one is the architecture agent); this file has nothing to do with
// those. dispatcher agent keys (agents-state.json, agent-dispatcher.py) are intentionally
// NOT renamed — that's opti's control-plane vocabulary and touches the GH workflow too.

// report name -> { label, agent (dispatcher key), cadence_h, home, order }
// `home: true` items are surfaced on the Home page, not in the #reports list.
// `order` fixes the card display order on the #reports page (lower = first):
...
router.get('/', (req, res) => {
  if (!fs.existsSync(AGENT_LOGS_DIR)) {
    return res.json({ runners: [], message: 'Agent logs directory not found' });
  }
  let files;
  try {
  ...
  // Fixed display order (Doctor → Hardware → Software → Network); unknown runners fall
  ...
  res.json({ runners, agent_logs_dir: AGENT_LOGS_DIR });
});
...
router.get('/:name/history', (req, res) => {
  ...
  if (!dir.startsWith(AGENT_LOGS_DIR + path.sep)) {
    return res.status(400).json({ error: 'Invalid runner name' });
  }
...
// GET /api/runners/:name — full JSON report (also serves the Home Leetify card)
