...
export default function QueryPage() {
  ...
  const runSql = (statement: string) => {
    setSql(statement);
    if(permitted)run.mutate(statement);
  };

  const onKeyDown = (e: React.KeyboardEvent) => {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
      e.preventDefault();
      if(permitted)run.mutate(sql);
    }
  };
  ...
  return (
    <div className="query-page"><p className="a-inline-notice">Explore the schema and prepare queries here. Arbitrary execution is disabled in preview. <a href={LEGACY+"/query"} target="_blank" rel="noreferrer">Open live query console ↗</a></p>
          ...
          <div className="glass card query-editor-card">
            ...
            <div className="query-actions">
              <button
                type="button"
                className="tb-btn"
                disabled={!permitted || run.isPending}
