...
export default function BotsPage() {
  ...
  return (
    <Tabs.Root value={tab} onValueChange={(v) => setParams({ tab: v }, { replace: true })}>
      <p className="a-inline-notice">Demo configuration and sends are simulated locally. Preview availability varies by bot. <a href={LEGACY+"/bots"} target="_blank" rel="noreferrer">Live bot tools ↗</a></p><Tabs.List className="bot-tabs glass">
        {BOTS.map((b) => (
    ...
    <div className="bot-panel">
      <section className="glass card bot-status" data-s={enabled ? 'ok' : undefined}>
        <header>
          ...
          <button className={`tb-btn toggle ${enabled ? 'on' : 'off'}`}
            disabled={readOnly || toggle.isPending} onClick={() => toggle.mutate(!enabled)}>
        ...
        <div className="w-actions">
          <button className="tb-btn" disabled={readOnly || send.isPending} onClick={() => send.mutate()}>
      ...
      <section className="glass card">
        ...
        <div className="w-actions">
          ...
          <button className="tb-btn primary" disabled={readOnly || save.isPending || !config.data} onClick={submit}>
