            ...
            {rows.map((r) => (
              <tr key={r.key} data-down={!r.up || undefined}>
                ...
                <td className="ct-actions">
                  ...
                  <button className="tb-btn sm" disabled={readOnly || busy.has(r.key)} title={`Restart on ${r.host}`}
                    onClick={() => restart(r)}>
                    <RotateCw /> Restart
                  </button>
                  {r.update && (
                    <button className="tb-btn sm" disabled={readOnly || busy.has(r.key)} title="Pull newest image & recreate"
                      onClick={() => update(r)}>
