...
export default function CockpitPage() {
  ...
  const rebootHost = async (host: string) => {
    ...
    setBusyFor(host, true);
    try {
      const d = await post<{ ok?: boolean; simulated?:boolean }>(`/api/agents/${host}/reboot`, undefined, 20_000);
      if(d?.simulated){setBusyFor(host,false);toast('Reboot simulation completed. No host was restarted.','ok');return}
      if (d?.ok) {
      ...
      } else if (host === 'rpi' && (err.status===502 || err.status===504 || err.status===undefined)) {
                ...
                {reachable && hasControls ? (
                  <>
                    <button className={`tb-btn danger${m.reboot_required ? ' hot' : ''}`}
                      disabled={readOnly || isBusy} onClick={() => rebootHost(host)}>Reboot</button>
                    <button className="tb-btn" disabled={readOnly || isBusy} onClick={() => aptUpgrade(host)}>Apt upgrade</button>
                    {(a!.allowed_units ?? []).map((u) => (
                      <button key={u} className="tb-btn" disabled={readOnly || isBusy}
                  ...
                  <button className="tb-btn" disabled={readOnly || isBusy} onClick={() => wakeHost(host)}><Zap /> Wake</button>
                ) : (
