...
import { Settings2, X, ArrowUp, ArrowDown, Plus, Minus } from 'lucide-react';
...
export function BoardGrid({
  ...
  // Keyboard/touch ordering edits only the current breakpoint. Stack the selected
  // order without overlaps; desktop widths and the other breakpoint are preserved.
  const reorder = (id: string, direction: number) => {
    const ordered = [...(layouts[bp] ?? [])].sort((a,b) => a.y-b.y || a.x-b.x);
    const from = ordered.findIndex(l => l.i === id);
    ...
    let y=0;
    onLayoutChange({...layouts,[bp]:ordered.map(l=>{const next={...l,x:0,y};y+=l.h;return next})});
  };
  const resize = (id:string,delta:number) => onLayoutChange({...layouts,[bp]:(layouts[bp]??[]).map(l=>l.i===id?{...l,h:Math.max(l.minH??1,l.h+delta)}:l)});
            ...
            <div key={w.id} className="board-item">
              {editMode && (
                <div className="w-edit-bar">
                  <button className="w-edit-btn" title="Move earlier (current screen layout)" aria-label={`Move ${w.type} earlier`} onClick={() => reorder(w.id,-1)}><ArrowUp/></button>
                  <button className="w-edit-btn" title="Move later (current screen layout)" aria-label={`Move ${w.type} later`} onClick={() => reorder(w.id,1)}><ArrowDown/></button>
                  <button className="w-edit-btn" title="Increase height" aria-label={`Increase ${w.type} height`} onClick={() => resize(w.id,1)}><Plus/></button>
                  <button className="w-edit-btn" title="Decrease height" aria-label={`Decrease ${w.type} height`} onClick={() => resize(w.id,-1)}><Minus/></button>
