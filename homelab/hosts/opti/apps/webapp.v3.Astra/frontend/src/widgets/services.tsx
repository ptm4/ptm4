  ...
  return (
    ...
    >
          ...
          {(top.data?.domains ?? []).map((d) => (
            <div className="kv-row" key={d.domain}>
              ...
              <span>
                ...
                {' '}
                <button className="link-btn" disabled={readOnly || allow.isPending} title="Allow domain (simulated in demo)"
                   onClick={() => allow.mutate(d.domain)}>allow</button>
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="w-actions">
        <button className="tb-btn" disabled={readOnly || toggle.isPending} onClick={() => toggle.mutate(!enabled)}>
