...
> **2026-09-06 rename note:** this folder is now `homelab/hosts/rpi/webapp.v2.legacy/`
> (the v3 rewrites live beside it in `webapp.v3.Fable/` and `webapp.v3.Astra/`). The
> checkout below is still correct as written — the `webapp-v1` tag carries the OLD
> `homelab/hosts/rpi/webapp/` path together with workflows that point at it, so restoring
> the tag restores a self-consistent v1 world. The deployed path on the Pi
> (`/srv/docker/compose/webapp/`) never changed. After a v1 restore, delete or leave
> `webapp.v2.legacy/` as you prefer; nothing deploys it once the workflows are the tag's.
