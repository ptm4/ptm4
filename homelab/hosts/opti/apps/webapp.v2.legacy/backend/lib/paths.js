...
// Depth corrected 2026-09-10: the app tier's source moved from homelab/hosts/rpi/
// to homelab/hosts/opti/apps/ (one level deeper), so HOMELAB_ROOT is four hops up
// from WEBAPP_ROOT, not three. Only the dev-mode fallback paths depend on this —
// in the container the env overrides and mounts win — which is exactly why a wrong
// count here would fail silently, in dev only, as empty report directories.
const WEBAPP_ROOT = path.join(__dirname, '..', '..');                  // homelab/hosts/opti/apps/webapp.v2.legacy
