...
// v3 owns /streams (SvelteKit route); the v1 player stays reachable at /legacy/streams/.
const LEGACY_DIRS = ['architecture', 'agents', 'agentic', 'samba'];
...
module.exports = fp(async function staticMounts(app) {
  ...
  // forever. Must be its own registration: a setHeaders callback gets overwritten by
  // the plugin's cache-control handling, a dedicated maxAge does not. Registered
  // regardless of the root's era; without dist the route just 404s.
  // (v2's Vite build used /assets/ — the mount moved with the framework.)
  if (haveDist) {
    app.register(fastifyStatic, {
      root: path.join(DIST_DIR, '_app', 'immutable'),
