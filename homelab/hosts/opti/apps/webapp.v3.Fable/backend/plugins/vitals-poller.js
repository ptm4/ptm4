...
module.exports = fp(async function vitalsPoller(app) {
  ...
  // The same rollup /api/vitals serves — pushed on the event bus after every poll
  // cycle so an open dashboard learns about a new sample without asking (v3 SSE).
  function rollup() {
    const hosts = {};
    for (const [host, st] of Object.entries(state)) {
      hosts[host] = {
  ...
  app.decorate('vitals', { state, POLL_MS, MAX_SAMPLES, LONG_BUCKET_S, LONG_MAX, rollup });
  ...
  app.addHook('onReady', async () => {
    ...
    loadSnapshots();
    const pollAll = () => {
      Promise.all(Object.keys(AGENT_HOSTS).map(pollHost)).then(() => {
        if (app.events) app.events.emit('vitals', rollup());
      });
    };
    pollAll();
