...
module.exports = async function runnersRoutes(app) {
  ...
  app.get('/:name/history', async (req, reply) => {
    ...
    if (!path.resolve(dir).startsWith(path.resolve(AGENT_LOGS_DIR) + path.sep)) {
  ...
  app.get('/:name/log', async (req, reply) => {
    ...
    if (!path.resolve(file).startsWith(path.resolve(AGENT_LOGS_DIR) + path.sep)) {
      return reply.code(400).send({ error: 'Invalid runner name' });
    }
  ...
  app.get('/:name', async (req, reply) => {
    ...
    if (!path.resolve(fullPath).startsWith(path.resolve(AGENT_LOGS_DIR))) {
      return reply.code(400).send({ error: 'Invalid runner name' });
    }
