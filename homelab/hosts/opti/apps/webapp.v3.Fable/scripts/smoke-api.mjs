...
const MANIFEST = [
  ...
  // v3 additions (2026-09-10): the event bus, fleet, jobs/audit, incidents, the
  // services join, homelab-db proxy + Docs, and the streams guide. 503s are legitimate
  // where the upstream (homelab-db, stream-station) may be absent off-box.
  { method: 'GET', path: '/api/events/status', expect: [200] },
  ...
  { method: 'GET', path: '/api/hldb/docs', expect: [200, 503] },
  { method: 'GET', path: '/api/streams/guide', expect: [200, 502] },
  { method: 'POST', path: '/api/streams/watch', body: {}, expect: [400, 500], note: 'invalid body is local validation; 500 = token unconfigured' },
  ...
  // v3 owns /streams as an SPA route; the v1 player moved under /legacy/.
  { method: 'GET', path: '/legacy/streams/', expect: [200], html: true },
];
