...
function open(qc: QueryClient) {
  ...
  // Jobs are the one stream a person is actively staring at, so the payload goes
  // straight into the store rather than invalidating a query and waiting for a refetch.
  // A step that takes 400ms must not be invisible because the round trip took longer.
  es.addEventListener('job', (e) => {
    try {
      jobStore.push(JSON.parse((e as MessageEvent).data));
      qc.invalidateQueries({ queryKey: ['jobs', 'audit'] });
      mark('job');
    } catch { /* malformed */ }
  });
