// The bot registry — five near-identical Discord bots share one API contract
// (status/config/send/preview), so the only real difference is which config fields
// each one has. Declaring those here means adding bot #6 is a table entry, not a page.
//
// Field types map to the generic form renderer on the Bots page. Fields NOT listed
...
// ── structured-editor payloads (BotExtras) ──────────────────────────────────
