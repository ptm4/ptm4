<script lang="ts">
  ...
  import AuditView from './_parts/AuditView.svelte';

  // Audit joined 2026-09-10: Stream and Incidents both say what the HOMELAB did;
  // Audit says what WE did to it, which is the question you ask when something
  // broke shortly after you pressed a button.
  type View = 'stream' | 'incidents' | 'audit';
...
<div class="feed-page">
  <div class="shead">
    <h2>Activity</h2>
    <span class="meta">
      {view === 'stream' ? 'everything, newest first'
        : view === 'audit' ? 'every action run from this dashboard, kept permanently'
        : 'findings correlated by host and time'}
    </span>
    <div class="bot-tabs right">
      ...
      <button type="button" class="bot-tab" class:active={view === 'audit'} onclick={() => selectView('audit')}>Audit</button>
    </div>
  </div>
