<script lang="ts">
  // Stream view — the homelab as one chronological feed: reports, heals, bot posts,
  // updates, backups, security, homelab.db changes and open incidents, newest first,
  // filterable by category and host. Moved verbatim from the old routes/feed/+page.svelte
  // body when Activity gained the Stream/Incidents toggle (2026-09-10); the Incidents
  // view lives beside this one in ./IncidentsView.svelte.
...
<div class="feed">
  {#if changes.isError || incidents.isError}
    <p class="faint" style="font-size: 11px; margin: 0">
      {changes.isError ? 'changes unavailable (homelab-db)' : ''}{changes.isError && incidents.isError ? ' · ' : ''}{incidents.isError ? 'incidents unavailable on this backend' : ''}
    </p>
  {/if}
  <div class="fbar">
