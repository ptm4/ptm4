<script lang="ts">
  ...
  import { spark, brailleSpark, meter, pad, padEnd, rate, uptime, toneFor } from '$lib/features/tui/glyphs';
  ...
  let width = $state(48);          // graph width in character cells, set from layout
  let wide = $state(true);
  ...
  onMount(() => {
    ...
    const measure = () => {
      ...
      wide = w >= 900;
      // ~7px per mono cell at 12px; leave room for the label + value columns.
      width = Math.max(16, Math.min(64, Math.floor((Math.min(w, 1400) - (wide ? 620 : 300)) / 7)));
    };
    measure();
...
<div class="tui">
  ...
  <div class="grid">
    <Panel title="cpu" meta="{range} · % busy, load1">
      {#each HOSTS as h (h)}
        {@const s = samplesOf(h)}
        {@const l = latestOf(h)}
        <div class="row" data-t={toneFor(l?.cpu_pct)}>
          <span class="nm">{padEnd(SHORT[h], NAME_W)}</span>
          <span class="graph">{brailleSpark(s.map((x) => x.cpu_pct), width, [0, 100])}</span>
    ...
    <Panel title="memory" meta="% used" tone="ok">
      {#each HOSTS as h (h)}
        {@const l = latestOf(h)}
        <div class="row" data-t={toneFor(l?.mem_pct)}>
          <span class="nm">{padEnd(SHORT[h], NAME_W)}</span>
          <span class="graph">{meter(l?.mem_pct, Math.max(12, Math.floor(width * 0.7)))}</span>
      ...
      {#each HOSTS as h (h)}
        ...
        <div class="row dim">
          <span class="nm">{padEnd(SHORT[h], NAME_W)}</span>
          <span class="graph">{spark(s.map((x) => x.mem_pct), width, [0, 100])}</span>
    ...
    <Panel title="network" meta="rx ▲ / tx ▼ per host">
      {#each HOSTS as h (h)}
        ...
        <div class="row">
          <span class="nm">{padEnd(SHORT[h], NAME_W)}</span>
          <span class="graph net-rx">{brailleSpark(s.map((x) => x.rx_bps), width)}</span>
          <span class="val">↓{rate(l?.rx_bps)}</span>
        </div>
        <div class="row dim">
          <span class="nm"></span>
          <span class="graph net-tx">{brailleSpark(s.map((x) => x.tx_bps), width)}</span>
          <span class="val">↑{rate(l?.tx_bps)}</span>
        </div>
      {/each}
    </Panel>

    <Panel title="temperature" meta="°C" tone="warn">
      {#each HOSTS as h (h)}
        {@const s = samplesOf(h)}
        {@const l = latestOf(h)}
        <div class="row" data-t={l?.temp_c == null ? 'dim' : l.temp_c >= 75 ? 'crit' : l.temp_c >= 65 ? 'warn' : 'ok'}>
          <span class="nm">{padEnd(SHORT[h], NAME_W)}</span>
          <span class="graph">{brailleSpark(s.map((x) => x.temp_c), width, [20, 90])}</span>
          <span class="val">{l?.temp_c == null ? '   —' : pad(l.temp_c.toFixed(0), 3) + '°'}</span>
        </div>
      {/each}
      <div class="note dim">scale 20–90 °C · a flat line at the floor is a host with no sensor</div>
    </Panel>

    <Panel title="storage" meta={live.data?.run_at ? `doctor ${relTime(live.data.run_at)}` : 'from collectors'}>
      {#each disks as d (d.host + d.label)}
        <div class="row" data-t={toneFor(d.pct)}>
          <span class="nm">{padEnd(SHORT[d.host], NAME_W)}</span>
          <span class="lbl">{padEnd(d.label, 6)}</span>
          <span class="graph">{meter(d.pct, Math.max(12, Math.floor(width * 0.6)))}</span>
    ...
    <Panel title="containers" meta="{fleet.up}/{fleet.total} up" tone={fleet.total && fleet.up < fleet.total ? 'crit' : 'ok'}>
      {#each containersByHost as c (c.host)}
        <div class="row" data-t={c.total && c.up === c.total ? 'ok' : 'crit'}>
          <span class="nm">{padEnd(SHORT[c.host] ?? c.host, NAME_W)}</span>
          <span class="graph">{meter(c.total ? (c.up / c.total) * 100 : null, Math.max(10, Math.floor(width * 0.45)))}</span>
