// The one fetch wrapper (port of v2 lib/api.ts). Contracts the rest of the app relies on:
//  - JSON in/out, absolute /api paths
//  - non-2xx bodies still carry useful JSON ({ error }) — surfaced via ApiError
//  - a dropped connection or bare 502/504 on /api/agents or /api/llama can mean "the
