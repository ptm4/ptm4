...
test('GET / serves the era-appropriate shell', async () => {
  // Before the SvelteKit build exists, root is the untouched v1 app; after, the v3
  // shell (hashed assets under /_app/immutable/).
  const haveDist = fs.existsSync(path.join(__dirname, '..', '..', 'frontend', 'dist', 'index.html'));
  const r = await app.inject({ method: 'GET', url: '/' });
  assert.equal(r.statusCode, 200);
  assert.match(r.headers['content-type'], /text\/html/);
  assert.match(r.body, haveDist ? /\/_app\/immutable\// : /app\.js\?v=/);
});
