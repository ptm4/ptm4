// Mission-control read-model — read-only endpoints that reshape data the container
// already has on disk. No SSH, no new collection: doctor/software reports come off
// /agent-logs (opti pool, :ro), security reports off /reports (:ro), and the per-host
// architecture-agent fragments via lib/arch-data's readFragments.
//
//   GET /api/containers    — fleet table: doctor containers ⋈ fragments ⋈ image_updates
//   GET /api/timers        — systemd timers from the fragments
...
//   GET /api/linkcheck     — server-side reachability probes for the app tiles
//
...
// line instead: unit is the token ending in ".timer".
...
// ── /linkcheck config ────────────────────────────────────────────────────────
// Server-side reachability for the app tiles. The browser cannot do this itself:
// the dashboard is https, the LAN services are http, and fetch() from a secure page
// to an insecure origin is blocked as mixed content before the request leaves.
//
// The probe list is FIXED here — never taken from the request — so this endpoint
// cannot be used as an SSRF proxy. One entry per ORIGIN; the frontend matches its
// links to results by origin too. `key` is the origin exactly as the frontend
// writes it; `target` is what this container actually dials when that differs
// (inside the container `rpi.lan` resolves to ::1, so those go by LAN IP).
const PROBE_ORIGINS = [
  ...
  // v3 additions — the Launchpad shows every UI, so every UI gets a probe
...
module.exports = async function dashboardRoutes(app) {
  ...
  app.get('/activity', async (req) => {
    ...
    // security agents — the *-latest.json in /reports
    try {
    ...
    return { events: events.slice(0, limit), generated_at: new Date().toISOString() };
  });

  // ── GET /api/trends ────────────────────────────────────────────────────────
  // Daily-resolution series from the dated doctor snapshots. Correct for slow-moving
  // numbers (pool fill, disk fill); sub-daily lives in /api/vitals.
  app.get('/trends', async (req) => {
    ...
    return { days: dates.length, pool, disks, generated_at: new Date().toISOString() };
  });

  // ── GET /api/linkcheck ─────────────────────────────────────────────────────
  app.get('/linkcheck', async () => {
