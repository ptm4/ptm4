...
router.get('/', async (req, res) => {
  if (!KUMA_API_KEY) {
    return res.status(503).json({ ok: false, error: 'KUMA_API_KEY not configured' });
  }
  try {
    ...
    if (!r.ok) {
      return res.status(502).json({ ok: false, error: `Kuma /metrics HTTP ${r.status}` });
    }
    ...
    res.status(502).json({ ok: false, error: `Kuma unreachable: ${e.message}` });
  }
});
