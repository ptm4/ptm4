...
import { useCapabilities, LEGACY } from '../astra/data';
...
export default function LlmPage() {
  const permitted=useCapabilities().data?.liveWrites===true;
  ...
  const offline = status.isError || status.data?.running===false;

  return (
    <div className="llm-page"><p className="a-inline-notice">Model commands and prompts are disabled in this local preview. <a href={LEGACY+"/llm"} target="_blank" rel="noreferrer">Open live LLM controls ↗</a></p>
        ...
        {(models.data?.models?.length ?? 0) > 0 && (
          <div className="w-actions">
            <select
              ...
              disabled={!permitted || switchModel.isPending}
            >
      ...
      <section className="glass card">
        ...
        <textarea
          ...
          onKeyDown={(e) => {
            if (permitted && e.key === 'Enter' && (e.metaKey || e.ctrlKey) && question.trim()) ask.mutate(question.trim());
          }}
        />
        <div className="w-actions">
          <button className="tb-btn primary" disabled={!permitted || ask.isPending || !question.trim()}
      ...
      <section className="glass card">
          ...
          {(runbooks.data?.runbooks ?? []).map((r) => (
            <div className="kv-row" key={r.name}>
              ...
              <span>
                ...
                {' · '}
                <button className="link-btn" disabled={!permitted} onClick={() => deleteRunbook.mutate(r.name)}>delete</button>
              </span>
            </div>
          ))}
        </div>
        {editing && (
          <div className="runbook-editor">
            ...
            <div className="w-actions">
              <button className="tb-btn primary" disabled={!permitted || saveRunbook.isPending}
                onClick={() => saveRunbook.mutate({ name: editing, content: draft })}>
                <Save /> Save
              </button>
              <button className="tb-btn" onClick={() => setEditing(null)}>Cancel</button>
              <button className="tb-btn danger" disabled={!permitted} onClick={() => { deleteRunbook.mutate(editing); setEditing(null); }}>
                <Trash2 /> Delete
              </button>
            </div>
          </div>
