...
import { useCapabilities } from '../astra/data';
...
function HoverVital({
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`Inspect ${id} measurements`}
      aria-expanded={active}
      onFocus={(e)=>{const r=e.currentTarget.getBoundingClientRect();setRect(r);onHover(id,r)}}
      onBlur={()=>onHover(null)}
      onClick={(e)=>{const r=e.currentTarget.getBoundingClientRect();setRect(r);onHover(id,r)}}
      onKeyDown={(e)=>{if(e.key==='Escape'){onHover(null)}else if(e.key==='Enter'||e.key===' '){e.preventDefault();const r=e.currentTarget.getBoundingClientRect();setRect(r);onHover(active?null:id,r)}}}
...
export function HostVitalsWidget({ options }: { options?: Record<string, unknown> }) {
  const readOnly=useCapabilities().data?.mode!=='demo';
  ...
  const lsKey = `astra-vitals-range:${host}`;
  ...
  return (
    ...
    >
      ...
      <div className="w-actions host-actions">
        {controllable ? (
          <>
            <button className="tb-btn sm danger" disabled={readOnly || actions.busy} onClick={actions.reboot}
              title={`Reboot ${host} (typed confirm)`}><Power /> Reboot</button>
            <button className="tb-btn sm" disabled={readOnly || actions.busy} onClick={actions.aptUpgrade}
