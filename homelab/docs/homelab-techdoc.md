...
lives in **`/etc/homelab/samba-red.conf`** (repo: `homelab/hosts/opti/samba/samba-red.conf`),
...
service. Nothing depends on the desktop. Setup checklist: `hosts/opti/setup-opti.md`.
...
`tools/collectors/_hosts.py`); it is host-agnostic, so the same command works run from a workstation.
...
```
homelab/hosts/opti/ (dispatcher + opti units) · homelab/tools/automation/ (cross-host autoupdate/reboot)
