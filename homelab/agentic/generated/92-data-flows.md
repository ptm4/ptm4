...
> Generated: `2026-08-23T17:12:27+00:00`
...
|---|---|---|---|---|---|
| **hl-arch-agent fleet** | opti, rpi, noblenumbat | `:8787 (push at 00:00 local)` | 24.0h | 13h ago | arch fragments, vitals |
| **Discord bot post freshness** | rpi | `GET /api/<bot>/status (via webapp)` | 0.5h | 0m ago | findings |
| **Homelab collectors** | opti | `GitHub Actions homelab-agents.yml` | 0.5h | 20m ago | agent-logs |
...
| **Curated repo facts** | git | `homelab/{agentic,tools/architecture}` | on demand | 8h ago | docs, arch graph |
| **Security auditors** | opti | `GitHub Actions homelab-agents.yml` | 24.0h | 8h ago | security-reports |
...
| Dataset | Host | Source | Cadence | Freshness | Consumers |
|---|---|---|---|---|---|
| **agent-logs reports** | opti | `<agent-logs>/*-latest.json + dated dirs` | 0.5h | 20m ago | webapp, session hook, homelab-db |
...
| **Docs corpus** | git | `runbooks, rules, skills, generated docs` | on demand | 8h ago | Claude sessions, homelab-db FTS |
| **Leetify CS2 stats** | opti | `<agent-logs>/leetify-latest.json` | on demand | 73d ago | cs2_matches, cs2_ratings |
...
| **security-reports** | opti | `<security-reports>/*-latest.json` | 24.0h | 8h ago | webapp, homelab-db |
...
| **MCP + JSON API** | opti | `:9100 (/api, /mcp)` | on demand | 41h ago | Claude Code, webapp |
...
| `agent_runs` | 306 |
| `findings` | 372 |
| `collector_metrics` | 13,019 |
| `docs` | 185 |
| `change_events` | 108 |
| `arch_nodes` | 68 |
| `raw_documents` | 5 |
| `vitals_samples` | 6,542 |
