...
WORKSPACE = {
    ...
    "note": (
        ...
        "or via a CIFS mount). homelab/agentic/ IS tracked in git as of 2026-07-25 — only "
        ".claude/ is ignored, and that is generated by probe.py --wire, so nothing unique "
        "lives there. Commit new rules/skills/hooks; uncommitted files exist only on disk."
    ),
    "access_paths": {
